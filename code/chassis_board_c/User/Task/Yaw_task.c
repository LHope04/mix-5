...
static void Yaw_mode_1() //锁云台模式
{
		...
		{			
			//云台目标yaw角度   =   云台当前yaw角度  +  遥控器通道值  +  鼠标通道值
			init_yaw = init_yaw  + rc_ctrl.rc.ch[0]/660.0 * 0.8 + ((mouse_x ) / 16384.00 * 20); 
			
			detel_calc(&init_yaw); //越界处理
								
			err_yaw = init_yaw - ins_yaw;  //小陀螺模式会让云台偏移，根据陀螺仪得到的角度与当前角度作差
			
			detel_calc(&err_yaw);  //越界处理
			
			//角度环PID
			if(err_yaw < -1 || err_yaw > 1) //保证云台
			{
